...
func (s *stub2) Status(_ context.Context) (*ipnstate.Status, error) {
	if s.status != nil {
		return s.status, nil
	}
...
func TestBlobRangeAndHead(t *testing.T) {
	...
	if resp.Header.Get("Accept-Ranges") != "bytes" {
		t.Fatalf("accept-ranges %s", resp.Header.Get("Accept-Ranges"))
	}
	if resp.ContentLength != 500 {
		t.Fatalf("len %d", resp.ContentLength)
	}
	// Range
	...
	if resp.Header.Get("Content-Range") != "bytes 100-199/500" {
		t.Fatalf("content-range %s", resp.Header.Get("Content-Range"))
	}
	body := make([]byte, 100)
	...
	if resp.StatusCode != 404 {
		t.Fatalf("traversal expected 404 got %d", resp.StatusCode)
	}
	resp.Body.Close()
}
